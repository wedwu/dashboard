...
const NetworkGraphv4 = () => {
  ...
  const drawOrthogonalPath = (x1, y1, x2, y2) => {
    ...
    // Second elbow position (where vertical meets horizontal to target)
    const elbowX = midX;
    const elbowY = y2;
    
    return { path, arrowPath, elbowX, elbowY };
  };
        ...
        <svg width="1200" height="800" className="w-full">
          ...
          <g>
            {edges.map((edge, i) => {
              ...
              const { path, arrowPath, elbowX, elbowY } = drawOrthogonalPath(source.x, source.y, target.x, target.y);
              ...
              return (
                <g key={i}>
                  ...
                  <path
                    ...
                    opacity="0.6"
                  />
                  {/* Warning icon at second elbow if source or target is down */}
                  {(isSourceDown || isTargetDown) && (
                    <g transform={`translate(${elbowX - 10}, ${elbowY - 10})`}>
