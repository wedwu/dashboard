...
const buildItemsA = (devices) => {
  const all = new Set(devices.map(d => d.id))
  const linked = new Set()
  devices.forEach(d => d.links?.forEach(l => linked.add(l)))
  const result = [...all].filter(id => !linked.has(id))
  console.log("itemsWithoutLinksToThem ==> ", result)
  return result
};
const buildItemsB = (devices) => {
  ...
  for (let i = 0; i < devices.length; i++) {
    ...
    if (deviceLinks && deviceLinks.length) {
      for (let j = 0; j < deviceLinks.length; j++) {
        ...
        const linkIndex = itemsWithoutLinksToThem.indexOf((link))
        if (linkIndex !== -1) {
...
const buildItemsC = (devices) => {
  const itemsWithoutLinksToThem = devices.map(d => d.id)
  for (let i = 0; i < devices.length; i++) {
    const device = devices[i]
    const deviceLinks = device.links
    if (!deviceLinks) continue
    for (let j = 0; j < deviceLinks.length; j++) {
      const linkId = deviceLinks[j]
      const idx = itemsWithoutLinksToThem.indexOf(linkId)
      if (idx !== -1) {
        itemsWithoutLinksToThem.splice(idx, 1)
      }
    }
  }
  return itemsWithoutLinksToThem
}
const buildItemsD = (devices) => {
  // Start with ALL device ids
  const noIncoming = new Set(devices.map(d => d.id))
  for (const device of devices) {
    if (!device.links) continue
    for (const targetId of device.links) {
      noIncoming.delete(targetId)
    }
  }
  return [...noIncoming]
}

const dd = diagramConfig.devices

const bA = buildItemsA(dd)
const bB = buildItemsA(dd)
const bC = buildItemsC(dd)
const bD = buildItemsD(dd)
